let mongoose,bcrypt;_602‍.x([["default",()=>_602‍.o]]);_602‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_602‍.w("bcrypt",[["default",["bcrypt"],function(v){bcrypt=v}]]);
...
userSchema.pre('save', function (next) {
  ...
  if (user.isModified('password')) {
    return bcrypt.hash(user.password, 12, function (err, hash) {
      if (err) {
        _602‍.g.console.log('BCRYPT HASH ERR ', err);
        return next(err);
      }
      user.password = hash;
      return next();
...
userSchema.methods.comparePassword = function (password, next) {
  bcrypt.compare(password, this.password, function (err, match) {
    if (err) {
      _602‍.g.console.log('COMPARE PASSWORD ERR', err);
      return next(err, false);
    }
    // if no err, we get null
    _602‍.g.console.log('MATCH PASSWORD', match);
    return next(null, match); // true
  });
};

_602‍.d(mongoose.model('User', userSchema));
